...
class SrRevealerFront extends RevSliderFunctions {
	...
	public function get_script_list(){
		$min = file_exists(RS_REVEALER_PLUGIN_PATH . 'public/js/' . $this->slug . '.js') ? '' : '.min';
		return array('revealer' => RS_REVEALER_PLUGIN_URL.'public/js/' . $this->slug . $min . '.js');
	}
	...
	public function enqueue_scripts(){
		...
		$this->script_enqueued = true;
		$min = file_exists(RS_REVEALER_PLUGIN_PATH . 'public/js/' . $this->slug . '.js') ? '' : '.min';
		wp_enqueue_script('revslider-'.$this->slug.'-addon', RS_REVEALER_PLUGIN_URL . "public/js/" . $this->slug . $min . ".js", '', RS_REVISION, array('strategy' => 'async'));
	}
	...
	public function add_addon_files($html, $export){
		...
		foreach($list ?? [] as $handle => $script){
			$script_path = str_replace(RS_REVEALER_PLUGIN_URL, RS_REVEALER_PLUGIN_PATH, $script);
			if(!$export->usepcl){
				$export->zip->addFile($script_path,  $export_path_js . $handle . '.js');
			}else{
				$export->pclzip->add($script_path, PCLZIP_OPT_REMOVE_PATH, RS_REVEALER_PLUGIN_PATH.'public/js/', PCLZIP_OPT_ADD_PATH, $export_path_js);
			}
