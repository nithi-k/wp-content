...
class AnpsImport extends AnpsFramework {
                    ...
                    foreach($widgets_data as $key3=>$item2) {
                        if($widget_id==$key3) {
                            if($widget_name=='nav_menu' || $widget_name=='anps_menu') {
                                $name = $this->_get_menu_name($item2);
                                if (!$name) continue;
                                $item2['nav_menu'] = $name;
                            }
    ...
    public function save_file_widgets() {
        ...
        ob_clean();
        echo $this->get_anps_widgets(); // PHPCS: XSS ok.
        exit;
    }
    /* Save file */
    ...
    public function import_theme_options($filename='') {
        if(isset($_FILES['import_file']['name']) && $_FILES['import_file']['name']!='') {
            ...
            if($data == null){
                echo '<div style="padding-top:5px;padding-bottom:5px;" class="alert alert-danger-style-2"><i class="fa fa-exclamation"></i><p style="color: white;font-size:20px;padding-top:0;">You must have one page builder activated.</p></div>';
                return;
            }
            foreach($data as $name=>$value) {
                update_option($name, $value);
    ...
    public function save_widgets_data($data) {
        ...
        foreach($data as $key_sidebar => $item_sidebar) {
            ...
            foreach($item_sidebar as $key_widget => $item_widget) {
                ...
                $widgets_data = get_option('widget_'.$widget_name) ?: array();
