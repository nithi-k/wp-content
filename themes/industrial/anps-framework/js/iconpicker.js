	...
	function createPicker() {
		...
		picker.append('<ul class="anps-iconpicker-list">');
	   	...
	   	icons.forEach(function(icon) {
	   		...
	   		$('.anps-iconpicker-list').append('<li title="' + iconName + '" data-icon="' + icon + '"><i class="fab ' + icon + '"></i><span>' + iconName + '<span></li>');
	   	});
...
(function($) {
    window.anpsInitColorPicker = function (widget) {
        ...
        var options = {
            change: function(event, ui) {
				...
				if ($('.postbox').length) {
					$('.postbox .widefat:first').trigger('change');
				}
			},
            clear: function() {
                window.colorChange(this);
    ...
    window.anpsColorPickerReady = function() {
		...
		$('.postbox:has(.color-field)').each(function () {
			anpsInitColorPicker($(this));
		});
    };
}(jQuery));
