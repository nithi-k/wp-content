...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		// NOTE: We want the setup wizard to be accessible regardles if it has been completed or not (see issue IN-81).
		// If requirements change, uncomment the following block.
	...
	public function admin_page() {
		...
		wp_enqueue_style( 'anps', trailingslashit( $this->base_url ) . $this->directory . '/assets/css/anps' . $suffix . '.css', array( 'wp-admin' ), MERLIN_VERSION );
		...
		wp_enqueue_script( 'anps', trailingslashit( $this->base_url ) . $this->directory . '/assets/js/anps' . $suffix . '.js', array( 'jquery-core' ), MERLIN_VERSION );
		...
		<div class="merlin__wrapper">
			...
			<?php
				// Because the completed check is being ignored, the "Disable Wizard" link needs to be disabled because it's not functional (see IN-81)
				/*
	...
	public function steps() {
		...
		// 	);
		// }
		// Show the pagebuilder importer, only if TGMPA is included.
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['pagebuilder'] = array(
				'name' => esc_html__( 'Anps Plugins', 'merlin-wp' ),
				'view' => array( $this, 'pagebuilder' ),
			);
		}
	...
	protected function welcome() {
		...
		$no        = $strings['btn-no'];
		$wl        = $strings['welcome-let'];
		?>

		<div class="merlin__content--transition">
			<img class="img-step-welcome" src="http://astudio.si/preview/merlin-img/step-img/Group%20279.svg"/>

			<h1><?php echo esc_html( sprintf( $header, $theme ) ); ?></h1>

			<p class="paragraph-welcome"><?php echo esc_html( sprintf( $paragraph, $theme ) ); ?></p>

			<h4><?php echo esc_html( sprintf( $wl ) ); ?></h4>
	...
	protected function child() {
		...
		<div class="merlin__content--transition">
			...
			<img class="img-step-child" src="http://astudio.si/preview/merlin-img/step-img/Layer%202.svg"/>
			...
			<p class="paragraph-welcome" id="child-theme-text"><?php echo esc_html( sprintf( $paragraph, $theme ) ); ?></p>
	...
	protected function plugins() {
		...
		<div class="merlin__content--transition">
			...
			<img class="img-step-plugins" src="http://astudio.si/preview/merlin-img/step-img/Layer%204.svg"/>
			...
			<p class="paragraph-welcome"><?php echo esc_html( $paragraph ); ?></p>
	...
	protected function pagebuilder() {
		...
		// Variables.
		$url    = wp_nonce_url( add_query_arg( array( 'pagebuilder' => 'go' ) ), 'merlin' );
		$method = '';
		$fields = array_keys( $_POST );
		$creds  = request_filesystem_credentials( esc_url_raw( $url ), $method, false, false, $fields );
		...
		// Split the plugins into required and recommended.
		foreach ( $plugins['all'] as $slug => $plugin ) {
			if ( strpos($plugin['name'], 'Page Builder') ) {
				$required_plugins[ $slug ] = $plugin;
			} else {
				// $recommended_plugins[ $slug ] = $plugin;
			}
		...
		// Text strings.
		$header    = $count ? $strings['pagebuilder-header'] : $strings['pagebuilder-success'];
		$paragraph = $count ? $strings['pagebuilder'] : $strings['pagebuilder-success%s'];
		$action    = $strings['pagebuilder-action-link'];
		$skip      = $strings['btn-skip'];
		$next      = $strings['btn-next'];
		$install   = $strings['btn-plugins-install'];
		?>
		...
		<div class="merlin__content--transition">
			...
			<img class="img-step-pagebuilder" src="http://astudio.si/preview/merlin-img/step-img/Layer%203.svg"/>
			...
			<?php if ( $count ) : ?>
				...
				<?php if ( ! empty( $required_plugins ) ) : ?>
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						...
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							<input type="checkbox" name="default_plugins[<?php echo esc_attr( $slug ); ?>]" class="checkbox anps_checkbox" id="default_plugins_<?php echo esc_attr( $slug ); ?>" value="0">
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								<i></i>
	...
	protected function content() {
		...
		<div class="merlin__content--transition">
			...
			<img class="img-step-content" src="http://astudio.si/preview/merlin-img/step-img/Layer%205.svg"/>
			...
			<div class="merlin__select-control-wrapper">
				...
				<select class="merlin__select-control js-merlin-demo-import-select anps_demo_import_select">
					<?php foreach ( $this->import_files as $index => $import_file ) : ?>
						<option value="<?php echo esc_attr( $index ); ?>"><?php echo esc_html( $import_file['import_file_name'] ); ?></option>
					<?php endforeach; ?>
				</select>
				<div id="anps_demo_img">
				<a href="<?php echo esc_attr( $this->import_files[0]['preview_url'] ); ?>" target="_blank"><img style="" class="anps-first-image-for-demo" src="<?php echo esc_html($this->import_files[0]["import_preview_image_url"])?>"/></a>
				<?php foreach ( $this->import_files as $index => $import_file ) : ?>
						<a href="<?php echo esc_attr( $import_file['preview_url'] ); ?>" target="_blank"><img style="display:none;" class="<?php echo esc_attr( $index ); ?> anps-image-for-demo" src="<?php echo esc_html( $import_file['import_preview_image_url'] ); ?>"/></a>
					<?php endforeach; ?>
				</div>
		...
		<form action="" method="post" class="<?php echo esc_attr( $multi_import ); ?>">
			...
			<ul style="display:none;" class="merlin__drawer merlin__drawer--import-content js-merlin-drawer-import-content">
	...
	protected function ready() {
		...
		<div class="merlin__content--transition">
			...
			<img class="img-step-ready" src="http://astudio.si/preview/merlin-img/step-img/Layer%206.svg"/>
	...
	public function generate_child() {
		if (!check_ajax_referer('merlin_nonce', 'wpnonce')) {
			wp_send_json(
				array(
					'error' => esc_html__( 'Generating child theme failed.', 'merlin-wp' ),
				)
			);
		}
	...
	public function generate_child_functions_php( $slug ) {
		...
		$output = "
			 ...
			 * This child theme was generated by Anps Themes.
			 *
	...
	function _ajax_content() {
		...
		$selected_import = intval(sanitize_text_field($_POST['selected_index']));
		$content_type = sanitize_text_field($_POST['content']);

		if (!$content) {
			$content = $this->get_import_data($selected_import);
		}

		if (!check_ajax_referer('merlin_nonce', 'wpnonce') || !isset($content[$content_type])) {
		...
		$proceed = isset($_POST['proceed']) ? $_POST['proceed'] === 'true' : false;
	...
	public function validate_import_file_info( $import_files ) {
		...
		if (is_array($import_files)){
			...
			$this->logger->warning(__('No page builder activated'));
		}
	...
	public function update_selected_import_data_info() {
		...
		$selected_index = isset($_POST['selected_index'])
			? intval(sanitize_text_field($_POST['selected_index']))
			: false;
