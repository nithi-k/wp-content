...
class AnpsFramework
{
    ...
    protected function set_option($arr = array(), $option_name = null)
    {
        ...
        foreach ($arr as $name => $value) {
            if ($option_name == 'google_fonts') {
                $arr_save[] = array('value' => urlencode($value['family']), 'name' => $value['family'], 'variants' => $value['variants']);
            } else {
    ...
    public function anps_create_color_option($option_name, $hex, $label, $description = "")
    {
        ...
        if (!isset($option_name, $hex, $label)) {
            return;
        }
    ...
    public function anps_create_select($option, $optionArray, $label = '', $default_val = '', $remove_option = '0', $dafault_title = '')
    {
        ...
        foreach ($optionArray as $key => $value) {
            if (get_option($option, $default_val) == $key) {
                $selected = ' selected';
            } else {
                $selected = '';
            }
 ...
 */
    ...
    {
        $output = '';
        $output .= '<div class="fixsave"><button type="submit">' . esc_html__('Save', 'industrial') . '<i class="far fa-save"></i></button></div>';
        $output .= '<button type="submit" class="bottom-save absolute"><i class="far fa-save"></i>' . esc_html__('Save all changes', 'industrial') . '</button>';
        echo wp_kses($output, array(
            'div' => array(
                'class' => array(),
            ),
